...
build-all: build/helm-charts-plugin

build/helm-charts-plugin: FORCE
	env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w $(GO_LDFLAGS)' -o build/helm-charts-plugin .
...
install: FORCE build/helm-charts-plugin
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/helm-charts-plugin "$(DESTDIR)$(PREFIX)/bin/helm-charts-plugin"
...
goimports: FORCE install-goimports
	@printf "\e[1;36m>> goimports -w -local https://github.com/sapcc/helm-charts-plugin\e[0m\n"
	@goimports -w -local github.com/sapcc/helm-charts-plugin $(patsubst $(shell awk '$$1 == "module" {print $$2}' go.mod)%,.%/*.go,$(shell go list ./...))
...
help: FORCE
	...
	@printf "  \e[36mbuild/helm-charts-plugin\e[0m     Build helm-charts-plugin.\n"
